'use client';
import { useState, useEffect } from 'react';
import { Plus } from 'lucide-react';
import { api, endpoints } from '@/lib/api';
import { TransactionModal } from './TransactionModal';
import Navbar from '@/components/layout/Navbar';

export default function TransactionsManager() {
  const [showForm, setShowForm] = useState(false);
  // 1. Initialiser explicitement comme un tableau vide
  const [transactions, setTransactions] = useState<any[]>([]);

  const fetchTransactions = () => {
    api.get(endpoints.transactions).then(res => {
      // 2. Vérifier la structure : si Django envoie { results: [...] }, on prend results
      const data = res.data.results || (Array.isArray(res.data) ? res.data : []);
      setTransactions(data);
    }).catch(err => {
      console.error("Erreur API :", err);
      setTransactions([]); // On vide le tableau en cas d'erreur
    });
  };

  useEffect(() => { fetchTransactions(); }, []);

  return (
     <div className="min-h-screen bg-gray-50">
              <Navbar />
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Transactions</h1>
          <p className="text-gray-500">Gérez les ventes et locations</p>
        </div>
        <button 
          onClick={() => setShowForm(true)}
          className="bg-blue-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-blue-700"
        >
          <Plus size={18} /> Nouvelle Transaction
        </button>
      </div>

      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-500 uppercase text-[10px] tracking-wider">
            <tr>
              <th className="px-6 py-4 text-left">Client</th>
              <th className="px-6 py-4 text-left">Type</th>
              <th className="px-6 py-4 text-left">Montant</th>
              <th className="px-6 py-4 text-right">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {/* 3. Sécurisation supplémentaire : double vérification */}
            {Array.isArray(transactions) && transactions.length > 0 ? (
              transactions.map((t: any) => (
                <tr key={t.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 font-bold">{t.client_name}</td>
                  <td className="px-6 py-4 capitalize">{t.type}</td>
                  <td className="px-6 py-4">{t.amount} €</td>
                  <td className="px-6 py-4 text-right">
                    <button className="text-blue-600 font-semibold hover:underline">Détails</button>
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan={4} className="px-6 py-10 text-center text-gray-400">
                  Aucune transaction trouvée.
                </td>
              </tr>
            )}
          </tbody>
        </table>
      </div>
      </div>


      {showForm && (
        <TransactionModal 
          onClose={() => setShowForm(false)} 
          onCreated={fetchTransactions} 
        />
      )}
    </div>
  );
}